...
// number in the form K000001, K000002, ... that never resets by year.
function genJobRef() {
  const rows = db.prepare("SELECT job_ref FROM reports WHERE job_ref LIKE 'K%'").all();
  let maxNum = 0;
  rows.forEach((r) => {
...
app.put('/api/reports/:id', requireAuth, requireRole('head'), (req, res) => {
  ...
  if (b.job_ref !== undefined) {
    ...
    if (newRef !== row.job_ref) {
      const clash = db.prepare('SELECT id FROM reports WHERE job_ref = ? AND id != ?').get(newRef, req.params.id);
      if (clash) {
  ...
  const fields = ['customer_name','customer_address','customer_email','customer_phone',
    'unit_location','unit_details','complaint_description','job_ref','written_name','team_members','amount',
